//! Sanctum Seeker — `{2}{B}{B}` 3/4 black Vampire Knight.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Sanctum Seeker");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_triggered_ability(TriggeredAbilityDef {
                id: 1,
                // GAP: vampire subtype filter not available at register time in ObjectFilter;
                // using creature().controlled_by(You) as closest approximation
