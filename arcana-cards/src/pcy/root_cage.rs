//! Root Cage — `{1}{G}` enchantment. "Mercenaries don't untap during
//! their controllers' untap steps."
//!
//! Implementation: an ETB trigger installs a
//! [`ContinuousEffect::filtered_dont_untap`] scoped to Mercenary
//! creatures (any controller), with duration
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Root Cage");
    // Intern "Mercenary" now so the effect fn's lookup is guaranteed to hit.
...
/// ETB trigger: install "Mercenaries don't untap during their
...
fn etb_install_untap_restriction(
    ...
    vec![Effect::InstallContinuousEffect {
        effect: ContinuousEffect::filtered_dont_untap(
            trig.source,
            ObjectFilter::creature().with_subtype_sym(mercenary),
