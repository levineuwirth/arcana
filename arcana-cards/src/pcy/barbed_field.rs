//! Barbed Field — `{2}{R}{R}` enchantment — Aura.
//! "Enchant land. Enchanted land has '{T}: This land deals 1 damage to any
//!  target.'"
//!
//! Host-activated Aura on a land. The ETB installs an `attached_activated`
//! continuous effect granting the enchanted land '{T}: deal 1 damage to any
//! target' — cost and effect run against the host land.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Barbed Field");
...
fn etb_install(
    ...
    vec![Effect::InstallContinuousEffect {
        effect: ContinuousEffect::attached_activated(
            trig.source,
            ActivatedAbilityDef {
                text: "{T}: This land deals 1 damage to any target.".into(),
                cost: ActivationCost {
                    tap: true,
                    ..Default::default()
                },
