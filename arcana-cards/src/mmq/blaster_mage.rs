//! Blaster Mage — `{2}{R}` 2/2 red Human Spellshaper.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Blaster Mage");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                text: "{R}, {T}, Discard a card: Destroy target Wall.".into(),
                cost: ActivationCost {
                ...
                target_requirements: vec![TargetRequirement {
                    filter: TargetFilter::Permanent(
                        ObjectFilter::permanent().with_subtypes_any(vec![
                            // Wall subtype filter
                        ]),
                    ),
