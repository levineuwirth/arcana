...
//!
//! The canonical control-change Aura (mirrors Corrupted Conscience). On ETB
//! (id 1) the engine has already attached the Aura, so the host is
//! `source.attached_to`; we `ChangeControl` it to the Aura's controller. When
//! the Aura leaves (id 2) we revert control to the host's owner.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Control Magic");
