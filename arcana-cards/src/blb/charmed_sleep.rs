//! Charmed Sleep — `{1}{U}{U}` enchantment — Aura.
...
//!
//! The ETB trigger taps the host (reached via `source.attached_to`) and
//! installs the `attached_dont_untap` lock — both faithful.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Charmed Sleep");
