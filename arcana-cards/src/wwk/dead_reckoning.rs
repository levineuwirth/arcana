...
//! from your graveyard on top of your library. If you do, Dead Reckoning deals
//! damage equal to that card's power to target creature."
//!
//! GAP: the "may" optional clause, conditional damage based on the topped
//! card's power, and multi-target interaction (one GY card + one creature) are
//! not expressible with the catalog. Best effort: put the graveyard card on
//! top of library; the damage clause is omitted.
...
fn resolve(
    ...
    // GAP: "may" optional clause and conditional damage equal to card's power not supported
    vec![Effect::PutOnTopOfLibrary { target: *gy_id }]
}
