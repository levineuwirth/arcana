//! Crusher Zendikon — `{2}{R}` enchantment — Aura.
//! "Enchant land. Enchanted land is a 4/2 red Beast creature with trample.
//!  It's still a land. When enchanted land dies, return that card to its
//!  owner's hand."
//!
//! The animation is a stack of ETB-installed attached effects on the host
//! land: add the Creature type, red color, Beast subtype, +4/+2, and
//! trample (the land keeps its land type). A host-dies trigger returns the
//! dying land card to its owner's hand.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Crusher Zendikon");
...
fn etb_animate(
    ...
    let mut beast_set = SubtypeSet::default();
    beast_set.0.insert(beast);
    vec![
