...
//!
//! Ward {2}.
//! I — Return each creature that isn't a Kraken, Leviathan, Merfolk, Octopus,
//!     or Serpent to its owner's hand. (GAP: "isn't one of these subtypes"
//!     not in ObjectFilter; returns all creatures as approximation.)
//! II, III — Whenever a Kraken, Leviathan, Merfolk, Octopus, or Serpent
//!            attacks, draw a card.
//!            (GAP: "whenever any of those subtypes attacks" not expressible
//!            in a single TriggerCondition; emitting Vec::new().)
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Summon: Leviathan");
    let saga_sub = reg.interner_mut().intern("Saga");
    let leviathan_sub = reg.interner_mut().intern("Leviathan");
    let mut subtypes = SubtypeSet::default();
    subtypes.0.insert(saga_sub);
    subtypes.0.insert(leviathan_sub);
...
fn chapter_i(state: &GameState, trig: &PendingTrigger, _r: &CardRegistry) -> Vec<Effect> {
    // GAP: "isn't Kraken/Leviathan/Merfolk/Octopus/Serpent" filter; bounce all creatures
    let filter = arcana_core::targets::ObjectFilter::creature()
        .controlled_by(ControllerConstraint::Opponent);
    let ids = script::ids_matching(state, &filter, trig.controller);
    ids.into_iter().map(|id| Effect::ReturnToHand { target: id }).collect()
}

fn chapter_ii_iii(_s: &GameState, _t: &PendingTrigger, _r: &CardRegistry) -> Vec<Effect> {
    // GAP: "whenever [sea creature subtypes] attacks, draw a card" this-turn-only trigger
    Vec::new()
}
