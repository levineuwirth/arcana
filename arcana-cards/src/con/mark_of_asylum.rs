//! Mark of Asylum — `{1}{W}` enchantment. "Prevent all noncombat
...
//! target filter "creature you control", `combat: Some(false)`
//! (noncombat only) — with `ReplacementKind::PreventAllDamage`,
//! lasting while the enchantment is on the battlefield.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Mark of Asylum");
...
/// ETB trigger: install "prevent all noncombat damage that would be
/// dealt to creatures you control" anchored to this enchantment.
