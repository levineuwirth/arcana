//! Goblin Scouts — `{3}{R}{R}` sorcery. "Create three 1/1 red Goblin
//! Scout creature tokens with mountainwalk."
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Goblin Scouts");
    let _goblin = reg.interner_mut().intern("Goblin");
    let _scout = reg.interner_mut().intern("Scout");
...
fn resolve(
    ...
    let scout = reg
        .interner()
        .lookup("Scout")
        .expect("Scout interned during register()");
    let mountain = reg
        .interner()
        .lookup("Mountain")
