//! Jungle Troll — `{1}{R}{G}` 2/1 Creature — Troll.
//!
//! Oracle:
//! * `{R}: Regenerate this creature.` — activated, mana-only cost.
//! * `{G}: Regenerate this creature.` — activated, mana-only cost.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Jungle Troll");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                text: "{R}: Regenerate this creature.".into(),
                cost: ActivationCost {
