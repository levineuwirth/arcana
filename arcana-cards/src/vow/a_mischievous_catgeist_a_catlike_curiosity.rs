...
//!   Disturb {1}{U} (cast from graveyard transformed).
...
//!
//! This is the Alchemy (A-) version with Disturb {1}{U} instead of {2}{U}.
//!
//! GAP: Disturb cast-from-graveyard mechanic not modeled (engine debt).
//! GAP: back-face Aura +1/+1 pump is a static layer on enchanted creature, not modeled.
//! GAP: back-face Aura granting ability to enchanted creature not modeled (static layer).
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("A-Mischievous Catgeist");
    ...
    let back_name = reg.interner_mut().intern("A-Catlike Curiosity");
