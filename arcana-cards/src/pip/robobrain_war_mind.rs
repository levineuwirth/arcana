...
//! a GAP (no CDA surface). The ETB energy gain is wired fully with a
//! dynamic amount. The attacks trigger is wired but its body is GAP'd:
//! OptionalPayment only supports Mana / Life, not an energy spend.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    ...
    // GAP: CDA "power is equal to the number of cards in your hand" —
    // no CDA surface; bones recorded with power as `*` (Star).
