...
//!
//! # GAP: Prowess keyword not in API keyword surface; "target spell or
//! permanent" combined filter not expressible (Permanent + Spell cannot
//! be OR'd in a single TargetRequirement).
...
fn resolve(
    ...
    let monk = reg.interner().lookup("Monk").expect("Monk interned during register()");
    let mut subtypes = SubtypeSet::default();
    subtypes.0.insert(monk);
    // GAP: Prowess keyword not in API; target[0] bounce uses any_target (best effort)
    ...
    // Bounce first target (spell or permanent — best effort: TargetChoice::Object)
    if let Some(TargetChoice::Object(id)) = entry.targets.targets.first() {
        effects.push(Effect::ReturnToHand { target: *id });
    }

    // Deal 4 damage to second target
