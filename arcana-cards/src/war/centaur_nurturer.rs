//! Centaur Nurturer — `{3}{G}` 2/4 Centaur Druid.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Centaur Nurturer");
...
fn add_any_color(_state: &GameState, ctx: &ActivationContext, _reg: &CardRegistry) -> Vec<Effect> {
    // GAP (fidelity): "one mana of any color" — the player's color choice is
    // not modeled; adds one green mana as best-effort.
    vec![Effect::AddMana {
