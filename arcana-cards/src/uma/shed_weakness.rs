...
//!
//! # GAP
//! No CounterKind variant for -1/-1 counters (catalog only lists PlusOnePlusOne).
...
fn resolve(
    ...
    // +2/+2 is expressible; -1/-1 counter removal is not (no MinusOneMinusOne CounterKind).
    // GAP: no CounterKind variant for -1/-1 counters
    vec![Effect::Pump {
        target: *id,
        power: 2,
        toughness: 2,
