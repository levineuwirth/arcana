//! Slimefoot, the Stowaway — `{1}{B}{G}` 2/3 Legendary Fungus.
//! "Whenever a Saproling you control dies, Slimefoot deals 1 damage to each
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Slimefoot, the Stowaway");
        ...
        CardDefinition::new(name, chars)
            .with_triggered_ability(TriggeredAbilityDef {
                ...
                trigger_condition: TriggerCondition::ZoneChange {
                    filter: saproling_filter,
                    from: Some(Zone::Battlefield),
                    to: Zone::Graveyard(0),
                },
                intervening_if: None,
                effect: on_saproling_dies,
                trigger_zones: vec![Zone::Battlefield],
                frequency: TriggerFrequency::EachTime,
                target_requirements: Vec::new(),
            })
            .with_activated_ability(ActivatedAbilityDef {
                text: "{4}: Create a 1/1 green Saproling creature token.".into(),
                cost: ActivationCost {
