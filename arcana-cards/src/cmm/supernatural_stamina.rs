...
//!
//! # GAP: granting a triggered ability to a creature until EOT
//! Effect::Pump handles +2/+0. Granting a triggered ability (on-death
//! return) is not available in the Effect catalog. Emitting Pump only.
...
fn resolve(
    ...
    // GAP: grant triggered "when dies return to battlefield tapped" ability until EOT
    vec![Effect::Pump {
        target: *id,
        power: 2,
        toughness: 0,
