//! Goblin Legionnaire — `{R}{W}` 2/2 Goblin Soldier.
//!
//! `{R}, Sacrifice this creature: It deals 2 damage to any target.`
//! `{W}, Sacrifice this creature: Prevent the next 2 damage that would
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Goblin Legionnaire");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                text: "{R}, Sacrifice this creature: It deals 2 damage to any target.".into(),
                cost: ActivationCost {
...
fn target_to_damage(target: &TargetChoice) -> Option<DamageTarget> {
    match target {
...
fn deal_two(_state: &GameState, ctx: &ActivationContext, _reg: &CardRegistry) -> Vec<Effect> {
    let Some(target) = ctx.targets.targets.first() else { return Vec::new(); };
    let Some(dt) = target_to_damage(target) else { return Vec::new(); };
...
fn prevent_two(_state: &GameState, ctx: &ActivationContext, _reg: &CardRegistry) -> Vec<Effect> {
    let Some(target) = ctx.targets.targets.first() else { return Vec::new(); };
    let Some(dt) = target_to_damage(target) else { return Vec::new(); };
