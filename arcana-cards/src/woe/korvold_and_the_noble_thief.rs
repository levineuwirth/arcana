//! Korvold and the Noble Thief — `{3}{R}` Enchantment — Saga
//!
//! I, II — Create a Treasure token.
//! III — Exile the top three cards of target opponent's library. You may play
//!        those cards this turn. (GAP: exile-with-play-permission not in
//!        Effect API; emitting Mill on opponent as approximation.)
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Korvold and the Noble Thief");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            ...
            .with_triggered_ability(TriggeredAbilityDef {
                ...
                intervening_if: None, effect: chapter_treasure,
                trigger_zones: vec![Zone::Battlefield], frequency: TriggerFrequency::EachTime, target_requirements: Vec::new(),
            })
            .with_triggered_ability(TriggeredAbilityDef {
...
fn chapter_treasure(_s: &GameState, trig: &PendingTrigger, _r: &CardRegistry) -> Vec<Effect> {
...
fn chapter_iii(_s: &GameState, trig: &PendingTrigger, _r: &CardRegistry) -> Vec<Effect> {
    ...
    // GAP: exile-with-play-permission not in Effect API
