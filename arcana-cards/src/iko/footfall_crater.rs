//! Footfall Crater — `{R}` enchantment — Aura.
//! "Enchant land. Enchanted land has '{T}: Target creature gains trample and
//!  haste until end of turn.' Cycling {1}."
//!
//! Host-activated grant: an ETB-installed `attached_activated` continuous
//! effect grants the enchanted land a tap ability that targets a creature and
//! grants it trample + haste until end of turn. Cycling {1} is a keyword.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Footfall Crater");
...
fn etb_install(_state: &GameState, trig: &PendingTrigger, _: &CardRegistry) -> Vec<Effect> {
    let ability = ActivatedAbilityDef {
        text: "{T}: Target creature gains trample and haste until end of turn.".into(),
        cost: ActivationCost {
