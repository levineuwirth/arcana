//! Terravore — `{1}{G}{G}` */* Lhurgoyf with Trample.
//!
//! Oracle:
//! * Trample.
...
//!   `self_pt_cda` counting land cards across every player's graveyard;
//!   symmetric `*`/`*`. No registry needed: a land is detected by its base
//!   type line.)
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Terravore");
...
fn install_cda(_state: &GameState, trig: &PendingTrigger, _reg: &CardRegistry) -> Vec<Effect> {
    vec![Effect::InstallContinuousEffect {
        effect: ContinuousEffect::self_pt_cda(
            trig.source,
            land_cards_in_all_graveyards,
            Duration::WhileSourceOnBattlefield,
        ),
    }]
}

fn land_cards_in_all_graveyards(s: &GameState, _source: ObjectId) -> (i32, i32) {
    let n = s
        .objects
