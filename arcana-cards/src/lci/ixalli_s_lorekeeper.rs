...
//! a Dinosaur source."
//! "Any color" is modeled as five mana abilities, one per WUBRG color
//! (command_tower idiom); the shared {T} cost means activating one taps the
//! source, so only one fires.
//! GAP: the spend-restriction ("only to cast a Dinosaur spell or activate an
//! ability of a Dinosaur source") is not modeled — ManaUnit carries no
//! spend-restriction tag.
