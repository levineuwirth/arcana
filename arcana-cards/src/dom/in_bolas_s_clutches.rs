//! In Bolas's Clutches — `{4}{U}{U}` legendary enchantment — Aura.
//! "Enchant permanent. You control enchanted permanent. Enchanted
...
//! `source.attached_to`; we `ChangeControl` it to the Aura's controller and
//! install an `add_supertypes(LEGENDARY)` continuous effect that follows the
//! host. When the Aura leaves (id 2) we revert control to the host's owner;
//! the legendary grant lapses with the Aura on the battlefield.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("In Bolas's Clutches");
