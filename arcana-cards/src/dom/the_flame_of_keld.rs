...
//! I — Discard your hand.
//! II — Draw two cards.
//! III — If a red source you control would deal damage to a permanent or
//!        player this turn, it deals that much damage plus 2 instead.
//!        (GAP: damage-replacement effect not in Effect API; emitting
//!        Vec::new().)
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("The Flame of Keld");
...
fn chapter_i(_state: &GameState, trig: &PendingTrigger, _reg: &CardRegistry) -> Vec<Effect> {
    vec![Effect::Discard { player: trig.controller, count: 7, choice: DiscardChoice::ControllerChooses }]
}
...
fn chapter_iii(_state: &GameState, _trig: &PendingTrigger, _reg: &CardRegistry) -> Vec<Effect> {
    // GAP: damage replacement effect not in Effect API
    Vec::new()
}
