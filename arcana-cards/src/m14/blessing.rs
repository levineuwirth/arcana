...
//!
//! The granted host activated ability is an ETB-installed `attached_activated`:
//! its {W} cost and +1/+1-until-end-of-turn pump run against the HOST
//! (`ctx.source`).
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Blessing");
...
fn etb_install(
    ...
    vec![Effect::InstallContinuousEffect {
        effect: ContinuousEffect::attached_activated(
            trig.source,
            ActivatedAbilityDef {
                text: "{W}: Enchanted creature gets +1/+1 until end of turn."
                    .into(),
                cost: ActivationCost {
