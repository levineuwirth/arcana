//! Steal Enchantment — `{U}{U}` enchantment — Aura.
//! "Enchant enchantment. You control enchanted enchantment."
//!
//! A control-change Aura (Control Magic class). The engine attaches the
//! Aura on resolution, so the host is `source.attached_to`. On ETB (id 1)
//! we `ChangeControl` the host to the Aura's controller; when the Aura
//! leaves (id 2) we revert control to the host's owner. The enchant target
//! is restricted to enchantments via permanent().with_types(ENCHANTMENT).
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Steal Enchantment");
