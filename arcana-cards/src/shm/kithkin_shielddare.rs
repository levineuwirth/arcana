//! Kithkin Shielddare — `{1}{W}` 1/1 white Kithkin Soldier.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Kithkin Shielddare");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                text: "{W}, {T}: Target blocking creature gets +2/+2 until end of turn.".into(),
                cost: ActivationCost {
                ...
                face_gate: None,
                effect: pump_blocking_creature,
            }),
    )
}

fn pump_blocking_creature(
    _state: &GameState,
