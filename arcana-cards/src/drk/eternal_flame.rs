...
//! target opponent or planeswalker and half X damage, rounded up, to you,
//! where X is the number of Mountains you control."
//! Planeswalker targeting is not a distinct TargetFilter; best effort uses
//! `any_target`. The "half X rounded up to you" is a second DealDamage.
//!
//! # GAP: planeswalker-specific targeting (no TargetFilter::Planeswalker)
...
fn resolve(
    ...
    let x = script::count_matching(state, &mountain_filter, entry.controller);
    let half_x = (x + 1) / 2;
    let dt = match target {
