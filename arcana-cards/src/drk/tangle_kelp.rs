//! Tangle Kelp — `{U}` enchantment — Aura.
//! "Enchant creature
...
//! conditional "doesn't untap … if it attacked last turn" lock has no
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Tangle Kelp");
...
fn etb_install(state: &GameState, trig: &PendingTrigger, _: &CardRegistry) -> Vec<Effect> {
    // GAP: conditional "doesn't untap … if it attacked during its controller's
    // last turn" has no attached_* builder; only the ETB tap is expressed.
