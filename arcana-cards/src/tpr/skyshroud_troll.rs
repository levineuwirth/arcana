//! Skyshroud Troll — `{2}{G}{G}` 3/3 Creature — Troll Giant.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Skyshroud Troll");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                ...
                cost: ActivationCost { mana_cost: ManaCost::parse("{1}{G}").unwrap(), ..ActivationCost::default() },
...
fn regen(_state: &GameState, ctx: &ActivationContext, _reg: &CardRegistry) -> Vec<Effect> {
