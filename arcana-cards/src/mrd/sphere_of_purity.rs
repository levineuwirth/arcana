//! Sphere of Purity — `{3}{W}` enchantment. "If an artifact would
//! deal damage to you, prevent 1 of that damage."
//!
//! Implementation: an ETB trigger installs a
//! `ReplacementCondition::WouldDealDamage` replacement scoped to
//! artifact sources with `ReplacementKind::PreventDamageUpTo(1)`
//! (applies per damage event, non-depleting), lasting while the
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Sphere of Purity");
...
/// ETB trigger: install "if an artifact would deal damage to you,
/// prevent 1 of that damage" anchored to this enchantment.
    ...
    vec![Effect::InstallReplacementEffect {
        effect: Box::new(ReplacementEffect {
            ...
            condition: ReplacementCondition::WouldDealDamage {
                source_filter: ObjectFilter::default().with_types(TypeLine::ARTIFACT.into()),
