//! Slith Bloodletter — `{B}{B}` 1/1 Slith.
//!
//! * "Whenever this creature deals combat damage to a player, put a
//!   +1/+1 counter on it." — a combat-damage-to-player trigger.
//! * `{1}{B}: Regenerate this creature.` — a regenerate activation.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Slith Bloodletter");
    ...
    // DamageDealt has no self-only source filter, so the source is
    // restricted by this card's name (closest single-object approximation).
    let mut self_filter = ObjectFilter::creature();
    self_filter.name = Some(name);
