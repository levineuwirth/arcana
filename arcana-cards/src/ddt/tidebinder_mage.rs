//! Tidebinder Mage — `{U}{U}` 2/2 Creature — Merfolk Wizard.
...
//! GAP: "does not untap" duration effect not in Effect catalog — Tap only
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Tidebinder Mage");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_triggered_ability(TriggeredAbilityDef {
                ...
                intervening_if: None,
                effect: tidebinder_mage_trigger,
...
fn tidebinder_mage_trigger(
    _state: &GameState,
    trig: &PendingTrigger,
    _reg: &CardRegistry,
) -> Vec<Effect> {
    // GAP: "doesn't untap during controller's untap step" not in Effect catalog
