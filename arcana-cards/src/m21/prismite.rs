...
pub fn register(reg: &mut CardRegistry) -> CardId {
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(mana_ability("{2}: Add {W}.", add_white_mana))
            .with_activated_ability(mana_ability("{2}: Add {U}.", add_blue_mana))
            .with_activated_ability(mana_ability("{2}: Add {B}.", add_black_mana))
            .with_activated_ability(mana_ability("{2}: Add {R}.", add_red_mana))
            .with_activated_ability(mana_ability("{2}: Add {G}.", add_green_mana)),
    )
}
