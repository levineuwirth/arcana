//! Clockwork Gnomes — `{4}` 2/2 colorless Artifact Creature — Gnome.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Clockwork Gnomes");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                text: "{3}, {T}: Regenerate target artifact creature.".into(),
                cost: ActivationCost {
                ...
                face_gate: None,
                effect: regenerate_artifact_creature,
            }),
    )
}

fn regenerate_artifact_creature(
    _state: &GameState,
