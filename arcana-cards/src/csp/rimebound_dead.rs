//! Rimebound Dead — `{B}` 1/1 Snow Creature — Skeleton.
//! `{S}: Regenerate this creature. ({S} can be paid with one mana from a snow source.)`
//! GAP: Snow mana symbol {S} not representable via ManaCost::parse; using {B} as placeholder.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Rimebound Dead");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                ...
                // GAP: {S} (snow mana) not representable; using {B} as cost placeholder
                cost: ActivationCost {
