...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Stern Marshal");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                text: "{T}: Target creature gets +2/+2 until end of turn.".into(),
...
fn pump(_state: &GameState, ctx: &ActivationContext, _reg: &CardRegistry) -> Vec<Effect> {
    ...
    vec![Effect::Pump { target: *id, power: 2, toughness: 2, duration: Duration::EndOfTurn, keywords: vec![] }]
}
