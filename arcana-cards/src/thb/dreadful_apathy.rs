//! Dreadful Apathy — `{2}{W}` enchantment — Aura.
//! "Enchant creature. Enchanted creature can't attack or block.
//! {2}{W}: Exile enchanted creature."
//!
//! Pacifism-style restriction (two `attached_cant_*` markers) plus the
//! Aura's own activated ability that exiles `source.attached_to`.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Dreadful Apathy");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            ...
            .with_activated_ability(ActivatedAbilityDef {
                text: "{2}{W}: Exile enchanted creature.".into(),
                cost: ActivationCost {
...
fn exile_enchanted(
    ...
    vec![Effect::ExilePermanent { target: host }]
}
