...
//! creature token. Each player chooses an artifact, a creature, and a
//! land from the next player's permanents; destroy each chosen.
...
fn resolve(
    ...
    let ogre = reg.interner().lookup("Ogre").expect("interned");
    ...
    // GAP: per-player turn-order interactive permanent picks from neighbor.
