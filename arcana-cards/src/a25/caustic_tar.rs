//! Caustic Tar — `{4}{B}{B}` enchantment — Aura.
...
//! player) runs against the enchanted land.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Caustic Tar");
...
fn etb_install_activated(
    ...
    let ability = ActivatedAbilityDef {
        text: "{T}: Target player loses 3 life.".into(),
        cost: ActivationCost {
