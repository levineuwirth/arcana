//! Highland Berserker — `{1}{R}` 2/1 red Human Berserker Ally.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Highland Berserker");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_triggered_ability(TriggeredAbilityDef {
                ...
                intervening_if: None,
                effect: ally_enters_first_strike,
...
fn ally_enters_first_strike(
    ...
    ally_ids.into_iter().map(|id| Effect::GrantKeyword {
        target: id,
