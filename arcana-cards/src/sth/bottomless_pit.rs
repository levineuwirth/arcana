//! Bottomless Pit — `{1}{B}{B}` enchantment.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Bottomless Pit");
...
/// "…that player discards a card at random." (your upkeep)
fn discard_you(
    _state: &GameState,
    trig: &PendingTrigger,
    _reg: &CardRegistry,
...
/// "…that player discards a card at random." (opponent's upkeep)
fn discard_opponent(
    state: &GameState,
    trig: &PendingTrigger,
    _reg: &CardRegistry,
