//! Protocol Knight — `{3}{U}` 3/4 blue Human Knight creature.
...
//!
//! GAP: stun counter kind is not in the CounterKind catalog; only the
//! Tap effect is expressed. Intervening-if condition "if you control
//! another Knight" is not expressible.
...
fn etb_tap_target(
    ...
    // GAP: stun counter not in CounterKind catalog; only Tap applied
