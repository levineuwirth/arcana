//! Phyrexian Archivist — `{6}` 4/5 Artifact Creature — Phyrexian Construct with Reach.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Phyrexian Archivist");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                text: "{2}, {T}: Put target card from a graveyard on the bottom of its owner's library.".into(),
                cost: ActivationCost {
