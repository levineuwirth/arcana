//! Albino Troll — `{1}{G}` 3/3 green Troll.
//!
//! Echo {1}{G}  — GAP: Echo is not in the usable KeywordAbility surface.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Albino Troll");
    let troll = reg.interner_mut().intern("Troll");
    let mut subtypes = SubtypeSet::default();
    subtypes.0.insert(troll);

    // GAP: Echo {1}{G} — Echo keyword not in the usable KeywordAbility surface.
