//! The Conundrum of Bowls — `{2}{U}{U}` blue Enchantment — Saga.
...
//! III — Count cards in hand. You may cast a spell with that mana value from your hand for free.
//! GAP: "Seek" (random search) not in catalog.
//! GAP: Chapter III "cast from hand for free with specific mana value" — free cast with CMC condition not in catalog.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("The Conundrum of Bowls");
    let saga_sub = reg.interner_mut().intern("Saga");
    let mut subtypes = SubtypeSet::default();
    subtypes.0.insert(saga_sub);
    let chars = Characteristics { name, mana_cost: Some(ManaCost::parse("{2}{U}{U}").expect("valid cost")), colors: ColorSet::blue(), types: TypeLine::ENCHANTMENT.into(), subtypes, supertypes: SupertypeSet::default(), ..Default::default() };
    reg.register(
...
fn chapter_gap(_state: &GameState, _trig: &PendingTrigger, _: &CardRegistry) -> Vec<Effect> {
    // GAP: Seek mechanic and free-cast-by-CMC not in catalog
    Vec::new()
}
