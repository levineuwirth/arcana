//! Blood Reckoning — `{3}{B}` enchantment.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Blood Reckoning");
    ...
    reg.register(
        CardDefinition::new(name, chars).with_triggered_ability(TriggeredAbilityDef {
            ...
            intervening_if: None,
            effect: bleed_attacker_controller,
...
/// "…that creature's controller loses 1 life." The attacker is
/// opponent-constrained by the trigger filter, so its controller is the
/// documented 2-player opponent read.
fn bleed_attacker_controller(
    state: &GameState,
    trig: &PendingTrigger,
    _reg: &CardRegistry,
) -> Vec<Effect> {
