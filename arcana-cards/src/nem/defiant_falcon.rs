//! Defiant Falcon — `{1}{W}` 1/1 Rebel Bird with Flying.
//! "{4}, {T}: Search your library for a Rebel permanent card with mana value
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Defiant Falcon");
...
fn tutor_rebel(_state: &GameState, ctx: &ActivationContext, reg: &CardRegistry) -> Vec<Effect> {
    // A Rebel permanent card with mana value 3 or less.
