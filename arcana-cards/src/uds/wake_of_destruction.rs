...
//! GAP: no filter for 'permanents with the same name as target'; ids_matching cannot
//! filter by name equality to a chosen target at resolution time.
...
fn resolve(
    ...
    // GAP: no filter for destroying all lands sharing the same name as the target
