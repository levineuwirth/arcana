//! Kresh the Bloodbraided — `{2}{B}{R}{G}` 3/3 Legendary Creature — Human Warrior.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Kresh the Bloodbraided");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_triggered_ability(TriggeredAbilityDef {
                ...
                intervening_if: None,
                effect: kresh_the_bloodbraided_trigger,
...
fn kresh_the_bloodbraided_trigger(
    state: &GameState,
    trig: &PendingTrigger,
    _reg: &CardRegistry,
) -> Vec<Effect> {
    ...
    vec![Effect::AddCounters { target: trig.source, kind: CounterKind::PlusOnePlusOne, count: n }]
}
