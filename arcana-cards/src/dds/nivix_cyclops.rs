//! Nivix Cyclops — `{1}{U}{R}` 1/4 Creature — Cyclops.
//! Defender.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Nivix Cyclops");
...
fn pump_self(_state: &GameState, trig: &PendingTrigger, _reg: &CardRegistry) -> Vec<Effect> {
    // "+3/+0 until end of turn" is expressed below. GAP: "can attack this turn
    // as though it didn't have defender" — no defender-ignoring rider Effect.
    vec![Effect::Pump {
        target: trig.source,
        power: 3,
        toughness: 0,
