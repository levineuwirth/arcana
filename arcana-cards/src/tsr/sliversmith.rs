//! Sliversmith — `{2}` 1/1 Artifact Creature — Spellshaper.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Sliversmith");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                text: "{1}, {T}, Discard a card: Create a 1/1 colorless Sliver artifact creature token named Metallic Sliver.".into(),
                cost: ActivationCost {
                ...
                face_gate: None,
                effect: create_sliver_token,
            }),
    )
}

fn create_sliver_token(
    _state: &GameState,
    ...
    token_subtypes.0.insert(sliver);
