//! Sporesower Thallid — `{2}{G}{G}` 4/4 Fungus.
//! "At the beginning of your upkeep, put a spore counter on each Fungus you
//! control."
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Sporesower Thallid");
...
fn spore_each_fungus(state: &GameState, trig: &PendingTrigger, reg: &CardRegistry) -> Vec<Effect> {
    let Some(spore) = reg.interner().lookup("spore").map(CounterKind::Named) else {
        return Vec::new();
    };
    let ids = script::ids_matching(
        state,
        &script::subtype_filter(reg, "Fungus").controlled_by(ControllerConstraint::You),
        trig.controller,
    );
    vec![Effect::ForEach {
        targets: ids,
