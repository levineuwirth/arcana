//! Poultice Sliver — `{2}{W}` 2/2 Sliver.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Poultice Sliver");
    ...
    // emitting tap-to-regenerate ability on self only.
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                text: "{2}, {T}: Regenerate target Sliver.".into(),
                cost: ActivationCost {
                ...
                face_gate: None,
                effect: regenerate_sliver,
            }),
    )
}

fn regenerate_sliver(
    _state: &GameState,
