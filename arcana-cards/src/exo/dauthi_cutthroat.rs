//! Dauthi Cutthroat — `{1}{B}` 1/1 Dauthi Minion with Shadow.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Dauthi Cutthroat");
    ...
    reg.register(
        CardDefinition::new(name, chars).with_activated_ability(ActivatedAbilityDef {
            text: "{1}{B}, {T}: Destroy target creature with shadow.".into(),
            cost: ActivationCost {
            ...
            face_gate: None,
            effect: destroy_shadow,
        }),
    )
}

fn destroy_shadow(_state: &GameState, ctx: &ActivationContext, _reg: &CardRegistry) -> Vec<Effect> {
