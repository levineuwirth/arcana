...
fn resolve(_state: &GameState, entry: &StackEntry, _reg: &CardRegistry) -> Vec<Effect> {
    ...
    if let Some(TargetChoice::Object(land)) = entry.targets.targets.get(1) {
        out.push(Effect::Tap { target: *land });
    }
    // GAP: "doesn't untap during its controller's next untap step" is
    // not expressible.
    out
}
