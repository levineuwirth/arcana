//! Ascent of the Worthy — `{1}{W}{B}` Enchantment — Saga
//!
//! I, II — Choose a creature you control. Until your next turn, all damage
//!          that would be dealt to creatures you control is dealt to that
//!          creature instead. (GAP: damage-redirection replacement effect
//!          not in Effect API; emitting Vec::new().)
//! III — Return target creature card from your graveyard to the battlefield
//!        with a flying counter on it. That creature is an Angel Warrior
//!        in addition to its other types.
//!        (GAP: type-granting and "flying counter" not in AddCounters—no
//!        CounterKind::Flying; emitting ReturnFromGraveyardToBattlefield only.)
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Ascent of the Worthy");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            ...
            .with_triggered_ability(TriggeredAbilityDef {
                ...
                intervening_if: None, effect: chapter_gap,
                trigger_zones: vec![Zone::Battlefield], frequency: TriggerFrequency::EachTime, target_requirements: Vec::new(),
            })
            .with_triggered_ability(TriggeredAbilityDef {
...
fn chapter_gap(_s: &GameState, _t: &PendingTrigger, _r: &CardRegistry) -> Vec<Effect> { Vec::new() }
