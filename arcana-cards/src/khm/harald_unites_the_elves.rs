//! Harald Unites the Elves — `{2}{B}{G}` black/green Enchantment — Saga.
//! I — Mill three cards. You may put an Elf or Tyvar card from your graveyard
//!     onto the battlefield.
//! II — Put a +1/+1 counter on each Elf you control.
//! III — Whenever an Elf you control attacks this turn, target creature an
//!        opponent controls gets -1/-1 until end of turn.
//! GAP: Chapter I "Elf or Tyvar" graveyard-put-to-BF combo not expressible
//! as a single effect (Reanimate searches whole GY not by specific card).
//! GAP: Chapter III "whenever Elf attacks this turn" is a delayed triggered
//! ability for the turn — not expressible.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Harald Unites the Elves");
...
fn chapter_i(_state: &GameState, trig: &PendingTrigger, reg: &CardRegistry) -> Vec<Effect> {
    // Mill 3, then best-effort: reanimate an Elf
...
fn chapter_iii(_s: &GameState, _t: &PendingTrigger, _r: &CardRegistry) -> Vec<Effect> {
    // GAP: "whenever an Elf attacks this turn, target creature opponent controls
    // gets -1/-1" — delayed within-turn trigger not expressible
    Vec::new()
}
