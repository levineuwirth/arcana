//! Rune of Mortality — `{1}{B}` enchantment — Aura Rune.
...
//!
//! Rune Aura. The ETB trigger draws a card and installs an attached
//! deathtouch grant (which naturally only affects a creature host). The
//! Equipment-grants-deathtouch nested clause is GAP'd.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Rune of Mortality");
...
fn etb_install(_state: &GameState, trig: &PendingTrigger, _: &CardRegistry) -> Vec<Effect> {
    ...
    // creature has deathtouch\"" nested grant is not expressible.
    vec![
        Effect::DrawCards {
