//! Blood Feud — `{4}{R}{R}` sorcery, "Target creature fights another
//! target creature." Two creature targets; each deals damage equal to its
//! power to the other.
...
fn resolve(
    ...
    let Some(a) = ids.next() else { return Vec::new(); };
    let Some(b) = ids.next() else { return Vec::new(); };
