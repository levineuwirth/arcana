//! Vitaspore Thallid — `{1}{G}` 1/1 Fungus.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    ...
    let sac_filter = ObjectFilter::default().with_subtype_sym(saproling_sym);
    ...
    reg.register(
        CardDefinition::new(name, chars)
            ...
            .with_activated_ability(ActivatedAbilityDef {
                text: "Sacrifice a Saproling: Target creature gains haste until end of turn.".into(),
                cost: ActivationCost {
...
fn upkeep_spore(_state: &GameState, trig: &PendingTrigger, reg: &CardRegistry) -> Vec<Effect> {
    let Some(spore) = reg.interner().lookup("spore").map(CounterKind::Named) else {
        return Vec::new();
    };
    vec![Effect::AddCounters { target: trig.source, kind: spore, count: 1 }]
}
