//! Pummeler for Hire — `{4}{G}` 4/4 Giant Mercenary with Vigilance, Reach, Ward {2}.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Pummeler for Hire");
...
fn etb_gain_life(_state: &GameState, _trig: &PendingTrigger, _reg: &CardRegistry) -> Vec<Effect> {
    // GAP: "X is the GREATEST POWER among Giants you control" — script has no
    // max-power-over-a-filter helper; only count/sum-style helpers are available.
    Vec::new()
}
