//! Shore Keeper — `{U}` 0/3 blue Trilobite.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Shore Keeper");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                text: "{7}{U}, {T}, Sacrifice this creature: Draw three cards.".into(),
                cost: ActivationCost {
                    mana_cost: ManaCost::parse("{7}{U}").unwrap(),
                    tap: true,
                    sacrifice: true,
