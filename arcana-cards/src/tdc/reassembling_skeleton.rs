//! Reassembling Skeleton — `{1}{B}` 1/1 black Skeleton Warrior.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Reassembling Skeleton");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                text: "{1}{B}: Return this card from your graveyard to the battlefield tapped.".into(),
                cost: ActivationCost {
...
fn reanimate_self(
    ...
    // GAP: activation zone should be Graveyard; using ReturnFromGraveyardToBattlefield
    vec![
