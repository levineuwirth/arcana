...
//!
//! Buff + host-trigger Aura: +2/+2 via attached_pt, and a host attack trigger
//! (`AttachedCreatureDoes { SelfAttacks }`) that creates a Treasure token.
//! "Goaded" is not in the keyword surface, so that clause is GAP'd.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Shiny Impetus");
...
fn etb_install(_state: &GameState, trig: &PendingTrigger, _: &CardRegistry) -> Vec<Effect> {
    // GAP: "is goaded" — Goad is not in the keyword surface. +2/+2 wired.
