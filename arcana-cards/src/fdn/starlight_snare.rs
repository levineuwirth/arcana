//! Starlight Snare — `{2}{U}` enchantment — Aura.
...
//!
//! ETB taps the host (read via `source.attached_to`); the don't-untap lock is
//! an `attached_dont_untap` continuous effect.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Starlight Snare");
