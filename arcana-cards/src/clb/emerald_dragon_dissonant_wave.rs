...
//! Wired via TargetFilter::AbilityOnStack (noncreature source_filter) + Effect::Counter,
//! which handles ability stack entries.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    ...
    let adv_ability = SpellAbilityDef { text: "Counter target activated or triggered ability from a noncreature source.".into(), target_requirements: vec![TargetRequirement { filter: TargetFilter::AbilityOnStack { activated: true, triggered: true, source_filter: Some(ObjectFilter::new().without_types(TypeLine::CREATURE.into())) }, count: TargetCount::Exactly(1), controller: None }], modal: None, effect: dissonant_wave_resolve };
...
fn dissonant_wave_resolve(_state: &GameState, entry: &StackEntry, _: &CardRegistry) -> Vec<Effect> {
