//! Flaming Fist — `{2}{W}` Legendary Enchantment — Background.
...
//! `SelfAttacks` (the granted creature) and grants it double strike
//! until end of turn. 'you own' is approximated by
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Flaming Fist");
...
/// ETB: install the commander grant of "Whenever this creature
    ...
    vec![Effect::InstallContinuousEffect {
        effect: ContinuousEffect::filtered_grant_triggered(
            ...
            TriggeredAbilityDef {
                ...
                intervening_if: None,
                effect: granted_attacks_double_strike,
                trigger_zones: vec![Zone::Battlefield],
                frequency: TriggerFrequency::EachTime,
                target_requirements: Vec::new(),
            },
...
/// Granted ability: the attacking commander gains double strike until
/// end of turn. `trig.source` is the granted (attacking) creature.
fn granted_attacks_double_strike(
    _state: &GameState,
    trig: &PendingTrigger,
    _: &CardRegistry,
) -> Vec<Effect> {
