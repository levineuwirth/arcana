//! Earthquake Dragon — `{14}{G}` 10/10 Elemental Dragon.
//!
//! This spell costs {X} less to cast, where X is the total mana value of
//! Dragons you control. (GAP — cost-reduction static.)
//! Flying, trample.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Earthquake Dragon");
    ...
    let chars = Characteristics {
        name,
        mana_cost: Some(ManaCost::parse("{14}{G}").expect("valid cost")),
    ...
    // GAP: "This spell costs {X} less to cast, where X is the total mana value
    // of Dragons you control" — a cost-reduction static on the spell; no
    // triggered/activated hook expresses it.
    reg.register(
        CardDefinition::new(name, chars).with_activated_ability(ActivatedAbilityDef {
            text: "{2}{G}, Sacrifice a land: Return this card from your graveyard to your hand."
                .into(),
            cost: ActivationCost {
