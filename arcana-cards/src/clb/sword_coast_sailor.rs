//! Sword Coast Sailor — `{1}{U}` Legendary Enchantment — Background.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Sword Coast Sailor");
    ...
    vec![Effect::InstallContinuousEffect {
        effect: ContinuousEffect::filtered_grant_triggered(
            ...
            TriggeredAbilityDef {
                ...
                intervening_if: None,
                effect: granted_attack_unblockable,
                trigger_zones: vec![Zone::Battlefield],
                frequency: TriggerFrequency::EachTime,
                target_requirements: Vec::new(),
            },
...
/// opponent has more life than that player, this creature can't be
/// blocked this turn." `trig.source` is the attacking commander;
/// the defender/life gate is checked against the firing event.
fn granted_attack_unblockable(
    state: &GameState,
    trig: &PendingTrigger,
    _: &CardRegistry,
) -> Vec<Effect> {
