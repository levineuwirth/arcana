//! CLB — set module. Auto-created by land_cards.py; add a proper set summary when curating.
pub mod chardalyn_dragon;
pub mod wyrm_s_crossing_patrol;
pub mod druidic_ritual;
pub mod armor_of_shadows;
pub mod wild_magic_surge;
pub mod bane_s_contingency;
pub mod summon_undead;
pub mod cast_down;
pub mod traverse_the_outlands;
pub mod breath_weapon;
pub mod earth_tremor;
pub mod in_garruk_s_wake;
pub mod split_the_spoils;
pub mod cut_a_deal;
pub mod syphon_mind;
pub mod marching_duodrone;
pub mod priest_of_ancient_lore;
pub mod flaming_fist_officer;
pub mod patron_of_the_arts;
pub mod owlbear_shepherd;
pub mod clockwork_fox;
pub mod roving_harper;
pub mod draconic_muralists;
pub mod mage_s_attendant;
pub mod high_priest_of_penance;
pub mod nemesis_of_reason;
pub mod carefree_swinemaster;
pub mod ingenious_artillerist;
pub mod bramble_sovereign;
pub mod malakir_blood_priest;
pub mod zhentarim_bandit;
pub mod treasure_keeper;
pub mod mother_of_runes;
pub mod warehouse_thief;
pub mod myrkul_s_invoker;
pub mod bane_s_invoker;
pub mod bonecaller_cleric;
pub mod dragonborn_looter;
pub mod bhaal_s_invoker;
pub mod baba_lysaga_night_witch;
pub mod reckless_barbarian;
pub mod silvanus_s_invoker;
pub mod sly_instigator;
pub mod order_of_whiteclay;
pub mod scaled_nurturer;
pub mod tymora_s_invoker;
pub mod alaundo_the_seer;
pub mod nimbleclaw_adept;
pub mod drillworks_mole;
pub mod spellskite;
pub mod steadfast_unicorn;
pub mod magus_of_the_balance;
pub mod the_akroan_war;
pub mod firja_s_retribution;
pub mod illithid_harvester_plant_tadpoles;
pub mod ettercap_web_shot;
pub mod dread_linnorm_scale_deflection;
pub mod tlincalli_hunter_retrieve_prey;
pub mod fang_dragon_forktail_sweep;
pub mod crystal_dragon_rob_the_hoard;
pub mod sapphire_dragon_psionic_pulse;
pub mod moonshae_pixie_pixie_dust;
pub mod sword_coast_serpent_capsizing_wave;
pub mod pegasus_guardian_rescue_the_foal;
pub mod hezrou_demonic_stench;
pub mod embereth_shieldbreaker_battle_display;
pub mod topaz_dragon_entropic_cloud;
pub mod guardian_naga_banishing_coils;
pub mod blessed_hippogriff_tyr_s_blessing;
pub mod gray_slaad_entropic_decay;
pub mod lovestruck_beast_heart_s_desire;
pub mod young_red_dragon_bathe_in_gold;
pub mod sea_hag_aquatic_ingress;
pub mod emerald_dragon_dissonant_wave;
pub mod young_blue_dragon_sand_augury;
pub mod colossal_badger_dig_deep;
pub mod amethyst_dragon_explosive_crystal;
pub mod artificer_class;
pub mod duke_ulder_ravengard;
pub mod astarion_s_thirst;
pub mod elminster_s_simulacrum;
pub mod irenicus_s_vile_duplication;
pub mod morphic_pool;
pub mod mindcrank;
pub mod nimblewright_schematic;
pub mod baldur_s_gate;
pub mod jade_orb_of_dragonkind;
pub mod basilisk_gate;
pub mod sky_diamond;
pub mod bountiful_promenade;
pub mod charcoal_diamond;
pub mod prized_statue;
pub mod saddle_of_the_cavalier;
pub mod blighted_woodland;
pub mod luxury_suite;
pub mod dimir_keyrune;
pub mod navigation_orb;
pub mod arcane_encyclopedia;
pub mod sea_of_clouds;
pub mod marble_diamond;
pub mod moss_diamond;
pub mod reflecting_pool;
pub mod cloak_of_the_bat;
pub mod universal_solvent;
pub mod terrain_generator;
pub mod lapis_orb_of_dragonkind;
pub mod stonespeaker_crystal;
pub mod mutavault;
pub mod fire_diamond;
pub mod sigil_of_myrkul;
pub mod goggles_of_night;
pub mod greatsword_of_tyr;
pub mod mystery_key;
pub mod warstorm_surge;
pub mod candlekeep_sage;
pub mod guild_artisan;
pub mod sword_coast_sailor;
pub mod agent_of_the_iron_throne;
pub mod flaming_fist;
pub mod cultist_of_the_absolute;
pub mod feywild_visitor;
pub mod stunning_strike;
pub mod stoneskin;
pub mod rowan_kenrith;
pub mod vivien_champion_of_the_wilds;
pub mod will_kenrith;
pub mod xenagos_the_reveler;
pub mod tasha_the_witch_queen;
pub mod korlessa_scale_singer;
pub mod pontiff_of_blight;
pub mod jon_irenicus_shattered_one;
pub mod sharpshooter_elf;
pub mod dawnbringer_cleric;
pub mod stirge;
pub mod nemesis_phoenix;
pub mod circle_of_the_land_druid;
pub mod livaan_cultist_of_tiamat;
pub mod undercellar_myconid;
pub mod skullport_merchant;
pub mod solemn_doomguide;
pub mod vhal_candlekeep_researcher;
pub mod druid_of_the_emerald_grove;
pub mod mardu_strike_leader;
pub mod wharf_infiltrator;
pub mod eight_and_a_half_tails;
pub mod bloodboil_sorcerer;
pub mod skanos_dragonheart;
pub mod wrathful_red_dragon;
pub mod phyrexian_revoker;
pub mod renari_merchant_of_marvels;
pub mod ellyn_harbreeze_busybody;
pub mod harper_recruiter;
pub mod raggadragga_goreguts_boss;
pub mod dusk_mangler;
pub mod angler_turtle;
pub mod ancient_bronze_dragon;
pub mod ancient_brass_dragon;
pub mod oji_the_exquisite_blade;
pub mod solemn_recruit;
pub mod uchuulon;
pub mod mocking_doppelganger;
pub mod durnan_of_the_yawning_portal;
pub mod dream_pillager;
pub mod ancient_silver_dragon;
pub mod sivriss_nightmare_speaker;
pub mod pseudodragon_familiar;
pub mod grim_hireling;
pub mod tabaxi_toucaneers;
pub mod elturel_survivors;
pub mod bloodsoaked_champion;
pub mod wilson_refined_grizzly;
pub mod hoarding_ogre;
pub mod ravenloft_adventurer;
pub mod erinis_gloom_stalker;
pub mod astarion_the_decadent;
pub mod ghastly_death_tyrant;
pub mod stirring_bard;
pub mod end_raze_forerunners;
pub mod rilsa_rael_kingpin;
pub mod nefarious_imp;
pub mod aarakocra_sneak;
pub mod sprite_dragon;
pub mod lae_zel_vlaakith_s_champion;
pub mod balor;
pub mod archpriest_of_iona;
pub mod sludge_monster;
pub mod geode_rager;
pub mod lurking_green_dragon;
pub mod gluntch_the_bestower;
pub mod beckoning_will_o_wisp;
pub mod minthara_merciless_soul;
pub mod wizards_of_thay;
pub mod alora_merry_thief;
pub mod kagha_shadow_archdruid;
pub mod scouting_hawk;
pub mod lulu_loyal_hollyphant;
pub mod seasoned_dungeoneer;
pub mod amber_gristle_o_maul;
pub mod lozhan_dragons_legacy;
pub mod felisa_fang_of_silverquill;
pub mod viconia_drow_apostate;
pub mod shadowheart_dark_justiciar;
pub mod nothic;
pub mod bronze_walrus;
pub mod nalia_de_arnise;
pub mod urabrask_the_hidden;
pub mod tiamat_s_fanatics;
pub mod white_plume_adventurer;
pub mod ryusei_the_falling_star;
pub mod dauthi_horror;
pub mod irregular_cohort;
pub mod bygone_bishop;
pub mod guiltfeeder;
pub mod burakos_party_leader;
pub mod deep_gnome_terramancer;
pub mod zevlor_elturel_exile;
pub mod sarevok_deathbringer;
pub mod genasi_enforcers;
pub mod battle_mammoth;
pub mod oceanus_dragon;
pub mod rescuer_chwinga;
pub mod bothersome_quasit;
pub mod githzerai_monk;
pub mod feywild_caretaker;
pub mod tomb_of_horrors_adventurer;
pub mod marut;
pub mod hammers_of_moradin;
pub mod safana_calimport_cutthroat;
pub mod frontline_medic;
pub mod dross_harvester;
pub mod aven_mindcensor;
pub mod warmonger_hellkite;
pub mod avenging_hunter;
pub mod sailors_bane;
pub mod faceless_one;
pub mod mold_folk;
pub mod cadira_caller_of_the_small;
pub mod intellect_devourer;
pub mod galepowder_mage;
pub mod hunted_horror;
pub mod gut_true_soul_zealot;
pub mod earthquake_dragon;
pub mod izzet_chemister;
pub mod astral_dragon;
pub mod thrakkus_the_butcher;
pub mod jan_jansen_chaos_crafter;
pub mod giant_ankheg;
pub mod elder_brain;
pub mod jaheira_friend_of_the_forest;
pub mod pursued_whale;
pub mod calculating_lich;
pub mod aurora_phoenix;
pub mod guildsworn_prowler;
pub mod zellix_sanity_flayer;
pub mod halsin_emerald_archdruid;
pub mod thunder_dragon;
