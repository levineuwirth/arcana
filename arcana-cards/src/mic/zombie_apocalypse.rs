...
//! creature cards from your graveyard to the battlefield tapped, then
//! destroy all Humans."
...
fn resolve(state: &GameState, entry: &StackEntry, reg: &CardRegistry) -> Vec<Effect> {
    // "Return all Zombie creature cards from your graveyard" is not
    // expressible (no graveyard-wide reanimation enumerator); emit the
    // "destroy all Humans" portion.
    let ids = script::ids_matching(state, &script::subtype_filter(reg, "Human"), entry.controller);
    vec![Effect::ForEach {
        targets: ids,
