//! Spire Barrage — `{4}{R}` sorcery, "Spire Barrage deals damage to any target equal to
...
fn resolve(
    ...
    let mountain_filter = script::subtype_filter(reg, "Mountain").controlled_by(ControllerConstraint::You);
    let count = script::count_matching(state, &mountain_filter, entry.controller);
    if count == 0 {
        return Vec::new();
    }
    let dt = match target {
