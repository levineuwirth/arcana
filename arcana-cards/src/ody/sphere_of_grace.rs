//! Sphere of Grace — `{3}{W}` enchantment. "If a black source would
//! deal damage to you, prevent 2 of that damage."
//!
//! Implementation: an ETB trigger installs a
//! `ReplacementCondition::WouldDealDamage` replacement scoped to black
//! sources with `ReplacementKind::PreventDamageUpTo(2)` (applies per
//! damage event, non-depleting), lasting while the enchantment is on
//! the battlefield.
//!
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Sphere of Grace");
...
/// ETB trigger: install "if a black source would deal damage to you,
/// prevent 2 of that damage" anchored to this enchantment.
    ...
    vec![Effect::InstallReplacementEffect {
        effect: Box::new(ReplacementEffect {
            ...
            condition: ReplacementCondition::WouldDealDamage {
                source_filter: ObjectFilter::default().with_colors(ColorSet::black()),
                target_filter: TargetFilter::Player,
                combat: None,
            },
            kind: ReplacementKind::PreventDamageUpTo(2),
