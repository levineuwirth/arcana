...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Ashen Firebeast");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            .with_activated_ability(ActivatedAbilityDef {
                text: "{1}{R}: This creature deals 1 damage to each creature without flying.".into(),
                cost: ActivationCost {
                ...
                face_gate: None,
                effect: damage_all_no_flying,
            }),
    )
}

fn damage_all_no_flying(
    state: &GameState,
