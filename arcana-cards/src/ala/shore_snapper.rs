//! Shore Snapper — `{2}{B}` 2/2 black Beast.
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Shore Snapper");
...
fn grant_islandwalk(
    ...
    let island_id = reg.interner().lookup("Island").expect("Island interned");
    vec![Effect::GrantKeyword {
        target: ctx.source,
        keyword: KeywordAbility::Landwalk(island_id),
