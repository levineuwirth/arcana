//! Ballad of the Black Flag — `{1}{U}{U}` Enchantment — Saga
//!
//! I, II, III — Mill three cards. You may put a historic card from among
//!              them into your hand. (GAP: "from among milled" retrieval
//!              not expressible; emitting only Mill.)
//! IV — Historic spells you cast this turn cost {2} less to cast.
//!       (GAP: cost reduction for this-turn-only deferred.)
...
pub fn register(reg: &mut CardRegistry) -> CardId {
    let name = reg.interner_mut().intern("Ballad of the Black Flag");
    ...
    reg.register(
        CardDefinition::new(name, chars)
            ...
            .with_triggered_ability(TriggeredAbilityDef {
                ...
                intervening_if: None, effect: chapter_mill,
                trigger_zones: vec![Zone::Battlefield], frequency: TriggerFrequency::EachTime, target_requirements: Vec::new(),
            })
            .with_triggered_ability(TriggeredAbilityDef {
...
fn chapter_mill(_s: &GameState, trig: &PendingTrigger, _r: &CardRegistry) -> Vec<Effect> {
    vec![Effect::Mill { player: trig.controller, count: 3 }]
}
fn chapter_iv(_s: &GameState, _t: &PendingTrigger, _r: &CardRegistry) -> Vec<Effect> { Vec::new() }
