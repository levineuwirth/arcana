...
- Keywords (unit variants, fully implemented): `KeywordAbility::Flying`, `Vigilance`, `Reach`, `Trample`, `Haste`, `Lifelink`, `Deathtouch`, `FirstStrike`, `DoubleStrike`, `Menace`, `Defender`, `Hexproof`, `Shroud`, `Indestructible`, `Flash`, `Fear`, `Intimidate`, `Shadow`, `Horsemanship`, `Skulk`. Place in `Characteristics.keywords` as a `Vec<KeywordAbility>`.
...
- Pre-wired marker keywords (recognized but rules NOT yet implemented): `Banding`, `Rampage`, `Bushido`, `Exalted`, `Soulshift`, `Unleash`, `Bloodthirst`, `Modular`, `Flanking`, `BattleCry`, `Undying`, `Persist`, `Afterlife`, `Mentor`, `Riot`, `Devour`, `Sunburst`, `Dethrone`, `Scavenge`, `Fading`, `Vanishing`, `Renown`, `Evolve`, `Graft`, `Provoke`, `Amplify`, `Enlist`, `Changeling`, `Infect`, `Wither`, `Toxic`. If the card's Scryfall keyword is one of these, DO emit it as `KeywordAbility::<Variant>` (unit variant, e.g. `KeywordAbility::Undying`) so the catalog records it. Scryfall `Battle cry` → `KeywordAbility::BattleCry`. These will be quarantined by the verify pipeline as "rules unimplemented" until a later pass — that is expected and correct; still emit the marker.
- The above (implemented + landwalk + pre-wired markers) is the COMPLETE usable keyword surface. Any other keyword (Cycling, Prowess, Ward, Protection, Convoke, etc.) is NOT available for this card class: emit `keywords: vec![]` and note the gap in the doc comment.
