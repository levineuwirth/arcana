...
- Keywords (unit variants, fully implemented): `KeywordAbility::Flying`, `Vigilance`, `Reach`, `Trample`, `Haste`, `Lifelink`, `Deathtouch`, `FirstStrike`, `DoubleStrike`, `Menace`, `Defender`, `Hexproof`, `Shroud`, `Indestructible`, `Flash`, `Fear`, `Intimidate`, `Shadow`, `Horsemanship`, `Skulk`, `Wither`, `Infect`, `Undying`, `Persist`, `Exalted`, `BattleCry`, `Mentor`, `Dethrone`, `Enlist`. Place in `Characteristics.keywords` as a `Vec<KeywordAbility>`. (Scryfall `Battle cry` → `KeywordAbility::BattleCry`.)
...
- Pre-wired marker keywords (recognized but rules NOT yet implemented): `Banding`, `Rampage`, `Bushido`, `Soulshift`, `Unleash`, `Bloodthirst`, `Modular`, `Flanking`, `Riot`, `Devour`, `Sunburst`, `Scavenge`, `Fading`, `Vanishing`, `Evolve`, `Graft`, `Provoke`, `Amplify`, `Changeling`. If the card's Scryfall keyword is one of these, DO emit it as `KeywordAbility::<Variant>` (unit variant, e.g. `KeywordAbility::Banding`) so the catalog records it. These will be quarantined by the verify pipeline as "rules unimplemented" until a later pass — that is expected and correct; still emit the marker.
