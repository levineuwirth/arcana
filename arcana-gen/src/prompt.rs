...
- `script::distinct_card_types_in_graveyard(state, p) -> u32`  (delirium — 'four or more card types', 'for each card type among cards in your graveyard') · `script::distinct_mana_values(state, &filter, {BINDING}.controller) -> u32`  ('for each different mana value among ~').
