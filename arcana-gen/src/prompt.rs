...
- Pre-wired marker keyword (recognized but rules NOT yet implemented): `Banding`. If the card's Scryfall keyword is `Banding`, DO emit it as `KeywordAbility::Banding` so the catalog records it. It will be quarantined by the verify pipeline as "rules unimplemented" (full combat banding is deferred) — that is expected and correct; still emit the marker.
