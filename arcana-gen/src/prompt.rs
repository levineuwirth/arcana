...
- `Effect::RollDie { player: p, sides: 20, outcomes: vec![RollOutcome { min: 1, max: 9, effect: Box::new(<a>) }, RollOutcome { min: 10, max: 19, effect: Box::new(<b>) }, RollOutcome { min: 20, max: 20, effect: Box::new(<c>) }] }`  — "Roll a d20" (AFR-style outcome table). `sides` = die faces (6, 20, …); each `RollOutcome` is an inclusive `[min, max]` result range → its effect; the FIRST matching range fires (order them, ranges are disjoint). Fair/deterministic/replayable. Wrap multi-step outcomes in `Effect::Sequence`. Use instead of GAP-ing dice rolls.
- CAST WITHOUT PAYING — `Effect::CastFromHandFree { player: p, target: id }` / `Effect::CastFromGraveyard { player: p, target: id }` / `Effect::CastFromExileFree { player: p, target: id }` — "cast that card without paying its mana cost" from hand / graveyard / exile respectively (Cascade/Discover hits, Flashback enablers, "exile target card and cast it"). No-op if the card isn't in the named zone; lands are skipped. The cast is mandatory — wrap "you MAY cast it" in a yes/no. Use instead of GAP-ing free casts.
- GRANT FLASHBACK — `Effect::GrantFlashback { target: id }` — "target instant or sorcery card in your graveyard gains flashback until end of turn; the flashback cost equals its mana cost" (Flashback the card / Recoup-class). Installs the flashback grant so the card becomes castable from the graveyard for its mana cost; pair with a `TargetRequirement` for the graveyard instant/sorcery. No-op if the target has no mana cost.
- "This spell can't be countered." — NOT an Effect: set `cant_be_countered: true` in the spell's own `Characteristics` block (defaults false). The flag rides onto the stack entry and counter effects no-op against it. Use instead of GAP-ing "can't be countered" on a spell.
...
- `ObjectFilter` refinements (chain onto `creature()` / `permanent()` / `subtype_filter(..)`): `.controlled_by(ControllerConstraint::You|Opponent)`, `.with_colors(ColorSet::black())`, `.without_colors(..)`, `.with_types_any(TypeLine(TypeLine::INSTANT | TypeLine::SORCERY))` (TYPE OR, for 'instant or sorcery' / 'creature or planeswalker'), `.without_types(TypeLine::ARTIFACT.into())`, `.with_max_cmc(n)` / `.with_min_cmc(n)`, `.with_min_power(n)` / `.with_max_power(n)` / `.with_max_toughness(n)`, `.tokens_only()` / `.nontoken()`, `.tapped_only()` / `.untapped_only()`, `.attacking_only()` / `.blocking_only()` / `.attacking_or_blocking_only()` / `.attacking_you_only()` / `.nonattacking_only()` / `.blocked_only()` (COMBAT STATUS, for 'each attacking creature' / 'target blocking creature' / 'creatures attacking you' / 'target blocked creature' — use these instead of GAP-ing combat-restricted filters), `.with_supertypes(SupertypeSet::new().with(SupertypeSet::LEGENDARY))` ('Legendary creature', 'Basic land', 'Snow permanent'), `.without_supertypes(SupertypeSet::new().with(SupertypeSet::LEGENDARY))` ('nonlegendary creature'), `.with_subtypes_any(vec![reg.interner_mut().intern("Human"), reg.interner_mut().intern("Warrior")])` (SUBTYPE OR — author this in `register` since it needs the interner; pass the resulting filter into the resolver via a closure). For a single subtype prefer `script::subtype_filter(reg, "Spirit")`.
- `script::power_of(state, id) -> i32` · `script::toughness_of(state, id) -> i32`.
- `script::source_counter_count(state, {BINDING}.source, CounterKind::Lore) -> u32`  — number of counters of a kind on the ability's OWN source ("for each [counter] on this", "X is the number of [counter] counters on ~"). The amount for counter-ACCUMULATION enchantments: "put a counter on this each upkeep, then [do X for each counter]" (Assemble the Legion: muster → that many 1/1 Soldiers; Descent into Avernus: descent → that many Treasures + that much damage; Mind Unbound: lore → draw that many). `CounterKind` ∈ `PlusOnePlusOne`/`Lore`/`Charge`/`Named(reg.interner_mut().intern("muster"))`/… Build the payoff by reading the count and baking it into the effect (`Effect::DrawCards{count: n}`; for "that many tokens"/"that many damage" emit one effect per count or per target). SAME-RESOLUTION ADD: when the SAME ability both adds the counter AND reads "for each counter on this" (the count includes the just-added one), this reads the count BEFORE the ability's own `AddCounters` applies — so add the printed increment: `let n = script::source_counter_count(state, {BINDING}.source, kind) + 2; vec![Effect::AddCounters{..count:2}, /* …use n… */]`. Use this instead of GAP-ing "for each [counter] on this".
...
- Per-turn event counters (use for "[N] died this turn" / "spells cast this turn" / "drew this turn" / "discarded this turn" — the turn slice resets on each new turn):
  ...
  · `script::creatures_died_this_turn(state) -> u32`  (ANY creature, all players — 'for each creature that died this turn')
  · `script::creatures_died_this_turn_controlled_by(state, p) -> u32`  (controller-scoped — 'for each creature YOU CONTROL that died this turn', Fresh Meat class; do NOT use the all-player count for 'your' deaths — it over-counts)
  · `script::life_lost_this_turn(state, p) -> u32`  ·  `script::life_gained_this_turn(state, p) -> u32`  ('the life you lost/gained this turn'; also the closest proxy for 'damage dealt to you this turn')
- `script::max_cmc_of(state, &filter, {BINDING}.controller) -> u32` · `script::max_power_of(state, &filter, {BINDING}.controller) -> i32`  — greatest mana value / power among permanents matching `filter` (0 if none); for 'equal to the greatest mana value/power among ~'.
- `script::distinct_card_types_in_graveyard(state, p) -> u32`  (delirium — 'four or more card types', 'for each card type among cards in your graveyard') · `script::distinct_mana_values(state, &filter, {BINDING}.controller) -> u32`  (different mana values among BATTLEFIELD permanents — 'for each different mana value among permanents you control') · `script::distinct_mana_values_in_graveyard(state, &filter, p, {BINDING}.controller) -> u32`  (the GRAVEYARD-scoped sibling — 'among cards in your graveyard'). Pick the one whose ZONE matches the oracle; do NOT use the battlefield one for a graveyard count.
- `script::name_of(state, id) -> Option<SmallString>`  (the name HANDLE, for 'a creature with the same name' — compare two handles for equality; no string compare) · `script::graveyard_ids_matching(state, &filter, p, {BINDING}.controller) -> Vec<ObjectId>`  (the IDs, not the count — feed `Effect::ForEach` for 'return ALL X from your graveyard').
- COMMODITY tokens: `script::subtype_filter(reg, "Treasure")` (also "Clue"/"Food"/"Powerstone"/"Incubator"/"Blood"/"Map") now MATCHES the minted token — use it for 'sacrifice a Treasure' / 'for each Food you control'. Do NOT GAP these.
...
- `Effect::RollDie {{ player: p, sides: 20, outcomes: vec![RollOutcome {{ min: 1, max: 9, effect: Box::new(<a>) }}, RollOutcome {{ min: 10, max: 19, effect: Box::new(<b>) }}, RollOutcome {{ min: 20, max: 20, effect: Box::new(<c>) }}] }}`  — 'Roll a d20' (AFR outcome table). `sides` = faces; each `RollOutcome` is an inclusive `[min, max]` result range → effect; FIRST matching range fires (disjoint, ordered). Deterministic/replayable; wrap multi-step outcomes in `Effect::Sequence`. Use instead of GAP-ing dice rolls.
- CAST WITHOUT PAYING — `Effect::CastFromHandFree {{ player: p, target: id }}` / `Effect::CastFromGraveyard {{ player: p, target: id }}` / `Effect::CastFromExileFree {{ player: p, target: id }}` — 'cast that card without paying its mana cost' from hand / graveyard / exile (Cascade/Discover hits, Flashback enablers, 'exile target card and cast it'). No-op if the card isn't in the named zone; lands skipped. Cast is mandatory — wrap 'you MAY cast' in a yes/no. Use instead of GAP-ing free casts.
- GRANT FLASHBACK — `Effect::GrantFlashback {{ target: id }}` — 'target instant or sorcery card in your graveyard gains flashback until end of turn; flashback cost = its mana cost' (Flashback the card / Recoup-class). Makes the card castable from the graveyard for its mana cost; pair with a `TargetRequirement` for the graveyard instant/sorcery. No-op if the target has no mana cost.
- \"This spell can't be countered.\" — NOT an Effect: set `cant_be_countered: true` in the spell's own `Characteristics` block (defaults false). The flag rides onto the stack entry and counter effects no-op against it. Use instead of GAP-ing 'can't be countered' on a spell.
...
- `script::max_cmc_of(state, &filter, entry.controller) -> u32` · `script::max_power_of(state, &filter, entry.controller) -> i32`  — the greatest mana value / power among battlefield permanents matching `filter` (0 if none). For 'equal to the greatest mana value/power among ~' (e.g. 'deals damage equal to the greatest power among creatures you control': `script::max_power_of(state, &ObjectFilter::creature().controlled_by(ControllerConstraint::You), entry.controller).max(0) as u32`).
- THIS-TURN event counts (read the turn's event log; use for 'equal to the number of ~ this turn'): `script::creatures_died_this_turn(state) -> u32` (all players) · `script::creatures_died_this_turn_controlled_by(state, p) -> u32` (controller-scoped — 'creatures YOU CONTROL that died', do NOT use the all-player count for 'your' deaths) · `script::life_lost_this_turn(state, p) -> u32` · `script::life_gained_this_turn(state, p) -> u32` · `script::cards_drawn_this_turn(state, p) -> u32` · `script::cards_discarded_this_turn(state, p) -> u32`. ('damage dealt to you this turn' is not separately tracked — `life_lost_this_turn` is the closest; use it for 'damage you've been dealt this turn'.) Use these instead of GAP-ing 'for each creature that died this turn' / 'X is the life you lost this turn' etc.
- GRAVEYARD / NAME / DISTINCT counts: `script::distinct_card_types_in_graveyard(state, p) -> u32` (delirium — 'four or more card types', 'for each card type in your graveyard') · `script::distinct_mana_values(state, &filter, entry.controller) -> u32` (different mana values among BATTLEFIELD permanents) · `script::distinct_mana_values_in_graveyard(state, &filter, p, entry.controller) -> u32` (GRAVEYARD-scoped — 'among cards in your graveyard'; match the oracle's ZONE, don't use the battlefield one for a graveyard count) · `script::name_of(state, id) -> Option<SmallString>` (name HANDLE for 'same name' matching — compare handles, never strings) · `script::graveyard_ids_matching(state, &filter, p, entry.controller) -> Vec<ObjectId>` (IDs not count — feed `Effect::ForEach` for 'return ALL X from your graveyard').
- COMMODITY tokens: `script::subtype_filter(reg, \"Treasure\")` (also \"Clue\"/\"Food\"/\"Powerstone\"/\"Incubator\"/\"Blood\"/\"Map\") MATCHES the minted token — use it for 'sacrifice a Treasure' / 'for each Food you control' / 'target Clue'. Do NOT GAP these.
...
- COUNTER DOUBLING / EXTRA COUNTERS ('If one or more counters would be placed on [filter], twice that many … instead' — Doubling Season / Branching Evolution / Vorinclex; '…that many plus one …' — Hardened Scales / Winding Constrictor) → `Effect::InstallReplacementEffect` with `condition: ReplacementCondition::WouldPlaceCounters {{ object_filter: <filter, e.g. ObjectFilter::creature().controlled_by(ControllerConstraint::You)>, kinds: CounterKindFilter::Only(CounterKind::PlusOnePlusOne) }}` (or `CounterKindFilter::Any` for 'any counter'), `kind: ReplacementKind::MultiplyCounters(2)` (double) or `ReplacementKind::AddAdditionalCounters(1)` (one more). Import `CounterKindFilter`, `CounterKind`. Do NOT GAP counter-doublers — fully supported.
- MANA-PRODUCTION DOUBLING ('If you tap a permanent for mana, it produces twice/three times as much … instead' — Mana Reflection = ×2, Nyxbloom Ascendancy = ×3) → `Effect::InstallReplacementEffect` with `condition: ReplacementCondition::WouldProduceMana {{ producer_filter: ObjectFilter::default(), player: ControllerConstraint::You }}, kind: ReplacementKind::MultiplyMana(2)`. Narrow `producer_filter` for 'tap a LAND/Forest for mana' variants. ONLY applies to mana ABILITIES (tapping); 'add {{N}} mana' spell effects are not multiplied. Stacks multiplicatively with other mana doublers.
...
- MAXIMUM HAND SIZE ('You have no maximum hand size' — Reliquary Tower / Thought Vessel / Spellbook; 'Your maximum hand size is N'; '+N maximum hand size') → `ContinuousEffect::max_hand_size(trig.source, <mod>, Duration::WhileSourceOnBattlefield)` where `<mod>` is `MaxHandSizeMod::NoMaximum` / `MaxHandSizeMod::SetTo(N)` / `MaxHandSizeMod::Delta(N)` (import `MaxHandSizeMod` from `arcana_core::layers`). Scoped to the source's controller ('you'); opponent-/all-player hand-size statics stay GAP'd.
- FLASH TIMING ('You may cast spells as though they had flash' — Vedalken Orrery / Leyline of Anticipation; 'You may cast creature spells as though they had flash') → `ContinuousEffect::cast_as_though_flash(trig.source, <spell ObjectFilter>, Duration::WhileSourceOnBattlefield)`. Use `ObjectFilter::default()` for ALL spells, or a type filter (`ObjectFilter::creature()`, types-only — matched against printed characteristics) to narrow. Scoped to the source's controller.
- SPELL-CAST LIMIT ('Each player can't cast more than one spell each turn' — Rule of Law / Eidolon of Rhetoric / Arcane Laboratory; 'more than one noncreature spell' — Deafening Silence; 'more than one artifact spell' — Ethersworn Canonist) → `ContinuousEffect::spell_cast_limit(trig.source, <spell ObjectFilter>, <ControllerConstraint — Any for 'each player', You/Opponent for one-sided>, N, Duration::WhileSourceOnBattlefield)`. The `<spell ObjectFilter>` narrows WHICH spells count: any spell = `ObjectFilter::default()`; NONCREATURE (Deafening Silence) = `ObjectFilter::new().without_types(TypeLine::CREATURE.into())`; a card type = `ObjectFilter::new().with_types_any(TypeLine::ARTIFACT.into())` (Ethersworn Canonist). Do NOT approximate 'noncreature' as 'any spell' — `without_types` expresses it exactly. Enforced in the legal-action cast gate against each player's per-turn cast count.
- PLAY FROM TOP OF LIBRARY ('You may play the top card of your library' — Future Sight; 'You may play lands from the top of your library' — Oracle of Mul Daya) → `ContinuousEffect::play_from_top_of_library(trig.source, Duration::WhileSourceOnBattlefield)`. Grants the source's controller play access to their library-top card (legal_actions offers it as a land play or a cast). The 'play with the top card revealed' info clause (Field of Dreams) is a view-only nicety and need not be separately modeled — if a card is ONLY reveal (no play), GAP the reveal.
- ANIMATE (board-wide 'are N/N creatures' — Nature's Revolt / Living Plane 'all lands are N/N creatures'; March of the Machines 'each noncreature artifact is a creature with P/T = its mana value' is the dynamic variant — GAP the dynamic-P/T half): install BOTH `ContinuousEffect::filtered_add_type(trig.source, <filter>, TypeLine::CREATURE.into(), Duration::WhileSourceOnBattlefield)` (adds the creature type, L4) AND `ContinuousEffect::filtered_set_base_pt(trig.source, <filter>, P, T, Duration::WhileSourceOnBattlefield)` (sets the body, L7b) — both with the SAME `<filter>`. Build `<filter>` to MATCH THE ORACLE EXACTLY: all lands = `ObjectFilter {{ types: Some(TypeLine::LAND.into()), ..Default::default() }}`; lands you control = `…controlled_by(ControllerConstraint::You)`; a LAND SUBTYPE ('all Forests' — Living Lands) = `script::subtype_filter(reg, \"Forest\")` (the effect fn receives `reg` — use it; do NOT widen 'Forests' to 'all lands'). They stay lands/artifacts (type is ADDED). Fixed P/T only — '+1/+1 counter'-derived or mana-value-derived bodies GAP the P/T half.
...
- 'Enchanted creature gets +X/+Y, where X is [a board count].' → `ContinuousEffect::attached_pt_dynamic(trig.source, <fn(&GameState, ObjectId) -> (i32, i32)>, Duration::WhileSourceOnBattlefield)` — a module-level fn that reads the SOURCE (the Aura) to count, like Empyrial Armor ('+1/+1 for each card in your hand'). For a NAMED-SUBTYPE or any filterable board count ('+1/+1 for each Forest you control' — Blanchwood Armor; '+1/+0 for each Mountain'; '+1/+1 for each creature/artifact you control') use `ContinuousEffect::attached_pt_per_match(trig.source, <ObjectFilter>, per_power, per_toughness, Duration::WhileSourceOnBattlefield)` — intern the subtype in `register()` and build `ObjectFilter::permanent().with_subtype_sym(forest).controlled_by(ControllerConstraint::You)` (or `.with_types(TypeLine::LAND.into())`, `ObjectFilter::creature().controlled_by(You)`, etc.); the engine counts matches from the host-controller's perspective and multiplies. Use `attached_pt_dynamic` (the bare fn) only for counts a filter can't express (cards in hand).
...
- 'Enchanted creature has PROTECTION from [X].' (Spirit Mantle, Holy Mantle, the Ward cycle) → `attached_keyword(trig.source, KeywordAbility::Protection(<ProtectionQuality>), Duration::WhileSourceOnBattlefield)` (import `ProtectionQuality` from `arcana_core::effects`, `Color` from `arcana_core::types`). Qualities: 'from red' → `ProtectionQuality::Color(Color::Red)`; 'from all colors' / 'from everything colored' → `ProtectionQuality::AnyColor`; 'from [creature type]' → `ProtectionQuality::CreatureType(reg.interner_mut().intern(\"Goblin\"))` (intern in register, capture the symbol); 'from everything' → `ProtectionQuality::Everything`. Protection is fully enforced (targeting, damage, blocking) — NOT a GAP. 'from two colors' (Sterling Grove-style) → two `attached_keyword` installs.
...
- CONTROL-CHANGE Auras ('You control enchanted creature' — Control Magic, Mind Control) are NOW SUPPORTED via a two-trigger mutate/revert (NOT an attached_* builder). Use `with_enchant(TargetFilter::Creature)` then TWO triggered abilities: (1) `id: 1`, `SelfEntersBattlefield` → effect returns `vec![Effect::ChangeControl {{ target: host, new_controller: trig.controller }}]` where `host = state.object_or_lki(trig.source).and_then(|o| o.attached_to)` (the engine attached the Aura before this ETB fires); (2) `id: 2`, `TriggerCondition::SelfLeavesBattlefield`, `trigger_zones: vec![Zone::Battlefield]` → effect reverts: look up `host` via the Aura's LKI attached_to, then `vec![Effect::ChangeControl {{ target: host, new_controller: <host's owner> }}]` (read `state.object_or_lki(host).owner`). Both effect fns guard with `let Some(host) = … else {{ return Vec::new(); }}`. (Control reverts to the creature's owner when the Aura leaves — CR 702 control-change.)
...
- `−X` loyalty ('−X: …' where X = the loyalty paid) IS expressible: `cost: ActivationCost {{ remove_loyalty_x: true, ..ActivationCost::default() }}` (do NOT also set remove_self_counter). The engine fans out one activation per X in 1..=current loyalty and threads the chosen X to the effect — read it with `ctx.x_value.unwrap_or(0)` and use it as the amount (e.g. `Effect::GainLife {{ player: ctx.controller, amount: ctx.x_value.unwrap_or(0) }}`, or reanimate creatures of mana value X, etc.).
...
PLANESWALKER ANIMATION (NOW SUPPORTED — 'this planeswalker becomes an N/N creature … it's still a planeswalker'): in the loyalty ability's effect fn return BOTH `Effect::AddType {{ target: ctx.source, types: TypeLine::CREATURE.into(), duration: Duration::EndOfTurn }}` and `Effect::SetBasePT {{ target: ctx.source, power: N, toughness: N, duration: Duration::EndOfTurn }}` (import `Duration` from arcana_core::layers). AddType is ADDITIVE so the planeswalker types are kept; the engine lets an object be creature+planeswalker at once. Add subtype/keyword grants the same way if the text names them.

EMBLEMS (NOW SUPPORTED — 'You get an emblem with \"…\"'): `Effect::CreateEmblem {{ controller: ctx.controller, emblem: EmblemDefinition {{ name: <interned emblem name>, statics: vec![…], abilities: vec![…] }} }}` (import `EmblemDefinition` from `arcana_core::effects`). Fill ONE of:
- STATIC emblem ('Creatures you control get +2/+2' — Elspeth, Gideon; 'have flying/trample') → `statics: vec![ContinuousEffect::anthem(arcana_core::objects::NULL_OBJECT_ID, ctx.controller, P, T, Duration::Permanent)]` (or `keyword_anthem(NULL_OBJECT_ID, ctx.controller, KeywordAbility::Flying, Duration::Permanent)` / `filtered_pump`/`filtered_keyword` for a filtered set). Use `NULL_OBJECT_ID` as the source placeholder — the engine repoints it at the emblem. `abilities: vec![]`.
- TRIGGERED emblem ('At the beginning of your end step, …' — Chandra Torch) → `abilities: vec![TriggeredAbilityDef {{ id: 1, trigger_condition: <e.g. TriggerCondition::StepBegins {{ step: Step::End, whose: ControllerConstraint::You }}>, intervening_if: None, effect: <fn>, trigger_zones: vec![Zone::Command], frequency: TriggerFrequency::EachTime, target_requirements: Vec::new() }}]`, `statics: vec![]`. (The engine remaps the id and forces the Command zone; write `trigger_zones: vec![Zone::Command]` to be safe.)
Intern the emblem name in `register()` (e.g. `reg.interner_mut().intern(\"<PW name> emblem\")`) and capture it. An emblem whose grant is a static the anthem/keyword/filtered builders can't express (rule-altering 'players can't…', 'spells cost less') still GAPs.

NOT EXPRESSIBLE — for a loyalty ability whose effect can't be built from the demonstrated `Effect` surface, still declare the ability with its correct loyalty COST but make its effect fn return `Vec::new()` with a `// GAP: <reason>` comment. Common GAPs: 'cast from exile / play this turn' riders, static/continuous abilities (no loyalty cost — those aren't loyalty abilities at all; if the PW's only text is a static, GAP the whole card), ultimates with bespoke one-shot effects, dynamic-X costs/effects. Declaring the ability shell (correct +/−/cost) is still valuable even when the effect is GAP'd. Emit AT LEAST the abilities you CAN express.
