...
- THIS-TURN event counts (read the turn's event log; use for 'equal to the number of ~ this turn'): `script::creatures_died_this_turn(state) -> u32` · `script::life_lost_this_turn(state, p) -> u32` · `script::life_gained_this_turn(state, p) -> u32` · `script::cards_drawn_this_turn(state, p) -> u32` · `script::cards_discarded_this_turn(state, p) -> u32`. ('damage dealt to you this turn' is not separately tracked — `life_lost_this_turn` is the closest; use it for 'damage you've been dealt this turn'.) Use these instead of GAP-ing 'for each creature that died this turn' / 'X is the life you lost this turn' etc.
