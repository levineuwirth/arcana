...
- `Effect::GrantTriggeredAbility {{ target: id, ability: Box::new(<TriggeredAbilityDef>), duration: Duration::EndOfTurn }}`  — grant a creature an ARBITRARY triggered ability ("until end of turn, whenever ~ deals combat damage to a player, draw a card" — Warrior's Lesson). Build the `TriggeredAbilityDef` exactly like a printed one (a fresh free `fn` for its `effect`), BUT its `id` MUST be in the granted range: `id: arcana_core::triggers::GRANTED_TRIGGER_ID_BASE + 1` (so the engine routes it). Import `use arcana_core::triggers::{{TriggeredAbilityDef, TriggerCondition, TriggerFrequency, GRANTED_TRIGGER_ID_BASE}};`. `duration` ∈ `Duration::EndOfTurn` (most "until end of turn" grants — auto-removed at cleanup) / `Duration::WhileSourceOnBattlefield` (static "has '<triggered ability>'"). For "target creature gains …" read `id` from `{BINDING}.targets.targets.first()`; for "~ gains …" use `{BINDING}.source`. Use this instead of GAP-ing "grant a triggered ability".
