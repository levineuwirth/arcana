...
- Keywords (unit variants, fully implemented): `KeywordAbility::Flying`, `Vigilance`, `Reach`, `Trample`, `Haste`, `Lifelink`, `Deathtouch`, `FirstStrike`, `DoubleStrike`, `Menace`, `Defender`, `Hexproof`, `Shroud`, `Indestructible`, `Flash`, `Fear`, `Intimidate`, `Shadow`, `Horsemanship`, `Skulk`, `Wither`, `Infect`, `Undying`, `Persist`, `Exalted`, `BattleCry`, `Mentor`, `Dethrone`, `Enlist`, `Flanking`, `Provoke`, `Sunburst`, `Unleash`, `Riot`, `Evolve`, `Soulshift`, `Scavenge`, `Changeling`. Place in `Characteristics.keywords` as a `Vec<KeywordAbility>`. (Scryfall `Battle cry` → `KeywordAbility::BattleCry`.)
...
- Cycling (parametrized, recognized): for Scryfall keyword `Cycling {cost}` (a mana cost in the rules text, e.g. "Cycling {2}"), emit `KeywordAbility::Cycling(ManaCost::parse("{2}").expect("valid cost"))`. (Typecycling/landcycling variants: just emit the generic `Cycling` with its printed cost; the type-search variant is not separately modeled.)
- Pre-wired marker keywords (recognized but rules NOT yet implemented): `Banding`, `Warp`. If the card's Scryfall keyword is one of these, DO emit it as `KeywordAbility::Banding` / `KeywordAbility::Warp` so the catalog records it. It will be quarantined by the verify pipeline as "rules unimplemented" (full combat banding / the warp cast mechanic are deferred) — that is expected and correct; still emit the marker.
