...
- `ObjectFilter` refinements (chain onto `creature()` / `permanent()` / `subtype_filter(..)`): `.controlled_by(ControllerConstraint::You|Opponent)`, `.with_colors(ColorSet::black())`, `.without_colors(ColorSet::black())` (nonblack), `.without_types(TypeLine::ARTIFACT.into())`, `.with_max_cmc(n)` / `.with_min_cmc(n)` / `.with_exact_cmc(n)`, `.with_min_power(n)` / `.with_max_power(n)` / `.with_max_toughness(n)`, `.tokens_only()` / `.nontoken()`, `.tapped_only()` / `.untapped_only()`. For 'each creature AND each planeswalker' (Star of Extinction-class wipes) use `ObjectFilter::permanent().with_types_any(TypeLine(TypeLine::CREATURE | TypeLine::PLANESWALKER))` — `with_types_any` matches if ANY of the bits is set, so the same filter catches both. Covers 'each creature with mana value 3 or less', 'all nonblack creatures', 'destroy target tapped creature', 'each token', etc. These same builders compose inside `TargetFilter::Permanent(..)` for filtered single targets — use them so 'target nonblack/tapped/power-N creature' is the actual target requirement, not an unfiltered creature.
...
- `Discard a card: …` / `Discard a [type] card: …` (a chosen card from your hand, NOT this card) → `ActivationCost { discard_other: Some(ObjectFilter::default()), ..ActivationCost::default() }`. Use `ObjectFilter::default()` for "a card"; add a `types`/`subtypes` filter for "a creature card" etc. The engine enumerates one activation per matching hand card. Single-card only — "discard two cards" is not expressible, GAP it.
