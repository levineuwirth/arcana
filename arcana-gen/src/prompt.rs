...
- SPELL-CAST LIMIT ('Each player can't cast more than one spell each turn' — Rule of Law / Eidolon of Rhetoric / Arcane Laboratory; 'more than one noncreature spell' — Deafening Silence; 'more than one artifact spell' — Ethersworn Canonist) → `ContinuousEffect::spell_cast_limit(trig.source, <spell ObjectFilter — ObjectFilter::default() for any, or types-only narrowing>, <ControllerConstraint — Any for 'each player', You/Opponent for one-sided>, N, Duration::WhileSourceOnBattlefield)`. Enforced in the legal-action cast gate against each player's per-turn cast count.
...
- ANIMATE (board-wide 'are N/N creatures' — Living Lands / Nature's Revolt 'all lands are 1/1 creatures'; March of the Machines 'each noncreature artifact is a creature with P/T = its mana value' is the dynamic variant — GAP the dynamic-P/T half): install BOTH `ContinuousEffect::filtered_add_type(trig.source, <filter>, TypeLine::CREATURE.into(), Duration::WhileSourceOnBattlefield)` (adds the creature type, L4) AND `ContinuousEffect::filtered_set_base_pt(trig.source, <filter>, P, T, Duration::WhileSourceOnBattlefield)` (sets the body, L7b). `<filter>` e.g. lands you control = `ObjectFilter {{ types: Some(TypeLine::LAND.into()), ..Default::default() }}.controlled_by(ControllerConstraint::You)`. They stay lands/artifacts (type is ADDED). Fixed P/T only — '+1/+1 counter'-derived or mana-value-derived bodies GAP the P/T half.
