...
- `Effect::DelayedAction {{ source: id, controller: entry.controller, when: DelayedWhen::NextEndStep, action: DelayedAction::Sacrifice }}`  — schedule a one-shot on a KNOWN id (a target you already have): when ∈ `NextEndStep` / `ThisDies`; action ∈ `Sacrifice` / `Exile` / `ReturnToHand`. Use for 'exile target creature, return it to its owner's hand at the beginning of the next end step' (`ExilePermanent` now + `DelayedAction{{when:NextEndStep, action:ReturnToHand}}`) and dies-on-target riders. NOT for 'create a token then sacrifice it' (the token id isn't available — GAP that). Import `DelayedWhen`, `DelayedAction` from `arcana_core::effects`.
