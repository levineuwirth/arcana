...
- Any other static ('Spells cost {{1}} more', 'Players can't gain life', 'nonbasic lands are Mountains') is still NOT expressible — the effect fn returns `Vec::new()` with a `// GAP: <which static is missing>` comment. The bones (name/cost/colors/types) are still valuable; emit them faithfully.
